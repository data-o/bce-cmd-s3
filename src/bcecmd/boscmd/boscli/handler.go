...
func (o *objectListIterator) listAllObjects(bucketName, objectKey, marker string, all,
	...
	for {
		response, err := o.bosClient.ListObjects(bucketName, delimiter, marker, objectKey, maxKeys)
		gtime := time.Now().Unix()
		...
		if !all || !(*response.IsTruncated) {
			...
			if response.NextMarker != nil {
				listRet.endInfo.nextMarker = *response.NextMarker
			}
			o.objectsChan <- listRet
			goto END
		} else {
...
func (o *objectListIterator) next() (*listFileResult, error) {
	...
	case <-time.After(time.Duration(GET_NEXT_OBJECT_TIME_OUT) * time.Millisecond):
...
func multiUploadNeedRetry(err error) bool {
	...
	if code == boscmd.CODE_NO_SUCH_UPLOAD || code == boscmd.CODE_INVALID_PART ||
		code == boscmd.CODE_INVALID_PART_ORDER {
		return true
	}
	return false
}
...
func (h *cliHandler) multiDeleteDir(bosClient bosClientInterface, bucketName,
	...
	var (
		...
		unDelObjects        []DeleteObjectResult
		curIndex            int
		err                 error
...
func (h *cliHandler) multiDeleteObjectsWithRetry(bosClient bosClientInterface, objectList []string,
	bucketName string) ([]DeleteObjectResult, error) {

	var (
		unDelList         []DeleteObjectResult
		unDelObjectsRetry *DeleteMultipleObjectsResult
		unDelObjects      *DeleteMultipleObjectsResult
		err               error
	)
...
func (h *cliHandler) utilCopyObject(srcBosClient, bosClient bosClientInterface, srcBucketName,
	...
	if fileSize > MULTI_COPY_THRESHOLD {
		...
		// common copy
		_, err = bosClient.CopyObject(dstBucketName, dstObjectKey, srcBucketName, srcObjectKey,
			storageClass)
	}
...
func (h *cliHandler) DownloadSuperFile(bosClient bosClientInterface, srcBucketName, srcObjectKey,
	...
	afterDownPartFail := func(err error) {
		...
		if code == boscmd.CODE_NO_SUCH_KEY && file != nil {
			file.Close()
			content.Remove()
		}
	}
...
func (h *cliHandler) utilUploadFile(bosClient bosClientInterface, srcPath, relSrcPath,
	...
	if fileSize > MULTI_UPLOAD_THRESHOLD {
		...
		_, err = bosClient.PutObjectFromFile(dstBucketName, dstObjectKey, relSrcPath, storageClass)
	}
...
func (h *cliHandler) UploadSuperFile(bosClient bosClientInterface, srcPath, dstBucketName,
	...
	if fileSize < MULTI_UPLOAD_THRESHOLD {
		_, err := bosClient.PutObjectFromFile(dstBucketName, dstObjectKey, srcPath, storageClass)
		return err
	}
	...
	if content.needRestart {
		uploadId, err := bosClient.InitiateMultipartUpload(dstBucketName, dstObjectKey, "",
			storageClass)
		if err != nil {
			return err
		}
	...
	uploadPart := func(bucket, object, uploadId string, partNumber int64, partBody []byte,
		result chan *CompletedPart, ret chan error, id int64, pool chan int64) {
	...
	uploadedResult := make(chan *CompletedPart, content.partsNum)
	...
	for partId := int64(1); partId <= content.partsNum; partId++ {
		if partInfo, ok := content.partIsFinish(partId); ok {
			uploadedResult <- &CompletedPart{
				ETag:       partInfo.ETag,
				PartNumber: partInfo.PartNumberId,
			}
	...
	completeArgs := NewCompletedMultipartUpload(content.partsNum)
	for i := content.partsNum; i > 0; i-- {
		...
		AddNewCompletedPart(completeArgs, uploaded, int(uploaded.PartNumber)-1)
...
func (h *cliHandler) doesBucketExist(bosClient bosClientInterface, bucketName string) (bool,
	...
	if code, ok := GetErrorCode(err); ok {
		...
		//fix this bug
		if code == boscmd.CODE_NO_SUCH_BUCKET {
			return false, nil
		} else if code == boscmd.CODE_ACCESS_DENIED {
			return true, nil
		}
	}
	return false, err
...
func (h *cliHandler) CopySuperFile(srcBosClient, bosClient bosClientInterface, srcBucketName,
	...
	// Do the parallel multipart upload
	if content.needRestart {
		uploadId, err := bosClient.InitiateMultipartUpload(dstBucketName, dstObjectKey, "", storageClass)
		if err != nil {
			return err
		}
		content.uploadId = uploadId
	...
	copyPart := func(srcBucket, srcObject, dstBucket, dstObject, uploadId string, partNumber,
		partSize, fileSize int64, result chan *CompletedPart, ret chan error, id int64,
		pool chan int64) {
		...
		log.Debugf("bos:/%s/%s => bos:/%s/%s start copy partNumber %d partSize %d range is %s , "+
			...
			partSize, sourceRange, rangeEnd-rangeStart)

		copyRet, err := bosClient.UploadPartCopy(dstBucketName, dstObjectKey, srcBucketName,
			srcObjectKey, uploadId, sourceRange, partNumber)
