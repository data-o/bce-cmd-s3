...
func TranUTCTimeStringToTimeStamp(utcTimeIn interface{}, oldTimeForm string) (int64, error) {
	switch utcTimeIn.(type) {
	case string:
		tempTime := utcTimeIn.(string)
		utcTime, err := time.Parse(oldTimeForm, tempTime)
		if err != nil {
			return 0, err
		}
	...
	default:
		tempTime := utcTimeIn.(*time.Time)
		timestamp := tempTime.Unix()
		return timestamp, nil
	}
}

// Change UTC time string to local time string
func TranUTCtoLocalTime(utcTimeIn interface{}, oldTimeForm, newTimeForm string) (string, error) {
	switch utcTimeIn.(type) {
	case string:
		tempTime := utcTimeIn.(string)
		utcTime, err := time.Parse(oldTimeForm, tempTime)
	...
	default:
		tempTime := utcTimeIn.(*time.Time)
		localTime := tempTime.Format(newTimeForm)
